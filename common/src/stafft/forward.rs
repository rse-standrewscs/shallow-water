...
pub fn forrdx6(
    ...
    assert_eq!(nv, a.len());
    assert_eq!(6, a[0].len());
    assert_eq!(lv, a[0][0].len());

    assert_eq!(nv, b.len());
    assert_eq!(lv, b[0].len());
    assert_eq!(6, b[0][0].len());

    assert_eq!(lv, cosine.len());
    assert_eq!(5, cosine[0].len());

    assert_eq!(lv, sine.len());
    assert_eq!(5, sine[0].len());
...
pub fn forrdx5(
    ...
    assert_eq!(nv, b.len());
    assert_eq!(lv, b[0].len());
    assert_eq!(5, b[0][0].len());

    assert_eq!(lv, cosine.len());
    assert_eq!(4, cosine[0].len());

    assert_eq!(lv, sine.len());
    assert_eq!(4, sine[0].len());
...
pub fn forrdx4(
    ...
    assert_eq!(nv, b.len());
    assert_eq!(lv, b[0].len());
    assert_eq!(4, b[0][0].len());

    assert_eq!(lv, cosine.len());
    assert_eq!(3, cosine[0].len());

    assert_eq!(lv, sine.len());
    assert_eq!(3, sine[0].len());
...
pub fn forrdx3(
    ...
    assert_eq!(nv, b.len());
    assert_eq!(lv, b[0].len());
    assert_eq!(3, b[0][0].len());

    assert_eq!(lv, cosine.len());
    assert_eq!(2, cosine[0].len());

    assert_eq!(lv, sine.len());
    assert_eq!(2, sine[0].len());
...
pub fn forrdx2(
    ...
    if nv < (lv - 1) / 2 {
        for i in 0..nv {
            for k in 1..=(lv - 1) / 2 {
                kc = lv - k;
                x1 = cosine[k - 1] * a[i][1][k] - sine[k - 1] * a[i][1][kc];
                y1 = cosine[k - 1] * a[i][1][kc] + sine[k - 1] * a[i][1][k];
                b[i][k][0] = a[i][0][k] + x1;
                b[i][kc][0] = a[i][0][k] - x1;
                b[i][k][1] = y1 - a[i][0][kc];
